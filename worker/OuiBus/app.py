...
from worker import config as tmw_api_keys
...
class OuiBusWorker(BaseWorker):
    ...
    def load_ouibus_database():
        ...
        try:
            ...
            bus_station_list["geoloc"] = bus_station_list.apply(
            ...
            try:
                logger.info(
                    os.path.join(os.getcwd(), "api/app/", "data/ouibus_stops.csv")
                )
                bus_station_list = pd.read_csv(
                    os.path.join(os.getcwd(), "api/app/", "data/ouibus_stops.csv"),
                    sep=",",
                )
                bus_station_list["geoloc"] = bus_station_list.apply(
                ...
                bus_station_list = pd.read_csv(
                    os.path.join(os.getcwd(), "app/", "data/ouibus_stops.csv")
                )
                bus_station_list["geoloc"] = bus_station_list.apply(
                    lambda x: [x.latitude, x.longitude], axis=1
                )
    ...
    def update_stop_list(retry=0):
        ...
        stops_rich = pandas_explode(
            stops_df_v2[["id", "stops", "_carrier_id"]], "stops"
        )
        stops_rich["stops"] = stops_rich.apply(lambda x: x.stops["id"], axis=1)
        stops_rich = stops_df_v1.merge(
            stops_rich,
            how="left",
            left_on="id",
            right_on="stops",
        ...
        stops_rich["id_meta_gare"] = stops_rich.id_meta_gare.combine_first(
            stops_rich.id
        )
        stops_rich["geoloc"] = stops_rich.apply(
            lambda x: [x.latitude, x.longitude], axis=1
        )

        logger.info(
            f"{stops_rich.shape[0]} Ouibus stops were found, here is an example:\n {stops_rich.sample()}"
        )
    ...
    def search_for_all_fares(date, origin_id, destination_id, passengers):
        ...
        timestamp = dt.now()
        r = requests.post(
            "https://api.idbus.com/v1/search", headers=headers, data=json.dumps(data)
        )
    ...
    def get_stops_from_geo_loc(geoloc_origin, geoloc_destination, max_distance_km=50):
        ...
        """
        stops_tmp = _ALL_BUS_STOPS[
            (
                ((_ALL_BUS_STOPS.latitude - geoloc_origin[0]) ** 2 < 0.6)
                & ((_ALL_BUS_STOPS.longitude - geoloc_origin[1]) ** 2 < 0.6)
            )
            | (
                ((_ALL_BUS_STOPS.latitude - geoloc_destination[0]) ** 2 < 0.6)
                & ((_ALL_BUS_STOPS.longitude - geoloc_destination[1]) ** 2 < 0.6)
            )
        ].copy()
        ...
        stops_tmp["distance_origin"] = stops_tmp.apply(
            lambda x: (x.latitude - geoloc_origin[0]) ** 2
            + (x.longitude - geoloc_origin[1]) ** 2,
            axis=1,
        )
        stops_tmp["distance_destination"] = stops_tmp.apply(
            lambda x: (x.latitude - geoloc_destination[0]) ** 2
            + (x.longitude - geoloc_destination[1]) ** 2,
            axis=1,
        )
        # We get the 5 closests station (within max_distance_km)
        stations = {}
        stations["origin"] = (
            stops_tmp[stops_tmp.distance_origin < 0.5]
            .sort_values(by="distance_origin")
            .head(5)
        )
        stations["destination"] = (
            stops_tmp[stops_tmp.distance_destination < 0.5]
            .sort_values(by="distance_destination")
            .head(5)
        )
        return stations
    ...
    def format_ouibus_response(df_response):
        ...
        df_response["duration_total"] = df_response.apply(
            lambda x: (x.arrival - x.departure).seconds, axis=1
        )
        response_rich = pandas_explode(df_response, "legs")
        response_rich["origin_id_seg"] = response_rich.apply(
            lambda x: x["legs"]["origin_id"], axis=1
        )
        response_rich["destination_id_seg"] = response_rich.apply(
            lambda x: x["legs"]["destination_id"], axis=1
        )
        response_rich["departure_seg"] = pd.to_datetime(
            response_rich.apply(lambda x: x["legs"]["departure"], axis=1)
        )
        response_rich["arrival_seg"] = pd.to_datetime(
            response_rich.apply(lambda x: x["legs"]["arrival"], axis=1)
        )
        response_rich["bus_number"] = response_rich.apply(
            lambda x: x["legs"]["bus_number"], axis=1
        )
        response_rich = response_rich.merge(
            _ALL_BUS_STOPS[["id", "geoloc", "short_name"]],
            left_on="origin_id_seg",
            right_on="id",
            suffixes=["", "_origin_seg"],
        )
        response_rich = response_rich.merge(
            _ALL_BUS_STOPS[["id", "geoloc", "short_name"]],
            left_on="destination_id_seg",
            right_on="id",
        ...
        response_rich = (
            response_rich.sort_values(by="price_cents")
            .head(limit)
    ...
    def compute_trips(date, passengers, geoloc_origin, geoloc_destination):
        ...
        for origin_meta_gare_id in origin_meta_gare_ids:
            origin_slug = all_stops["origin"][
                all_stops["origin"].id_meta_gare == origin_meta_gare_id
            ]._carrier_id_meta_gare.unique()[0]
            if pd.isna(origin_slug):
                origin_slug = all_stops["origin"][
                    all_stops["origin"].id_meta_gare == origin_meta_gare_id
                ]._carrier_id.unique()[0]
            for destination_meta_gare_id in destination_meta_gare_ids:
                destination_slug = all_stops["destination"][
                    all_stops["destination"].id_meta_gare == destination_meta_gare_id
                ]._carrier_id_meta_gare.unique()[0]
                if pd.isna(destination_slug):
                    destination_slug = all_stops["destination"][
                        all_stops["destination"].id_meta_gare
                ...
                if origin_meta_gare_id != destination_meta_gare_id:
                    all_fares = search_for_all_fares(
                        date, origin_meta_gare_id, destination_meta_gare_id, passengers
                    )
    ...
    def ouibus_journeys(df_response, _id=0):
        ...
        df_response = df_response.drop_duplicates(
            ["id", "arrival", "departure", "id_destination", "id_origin"]
        )
        ...
        df_response.insert(
            ...
            "price_step",
            df_response.apply(
                lambda x: x["price_cents"] / (x["nb_segments"] * 100), axis=1
            ),
        )
        ...
        for itinerary_id in df_response.id.unique():
            ...
            for index, leg in itinerary.iterrows():
                ...
                local_emissions = (
                    calculate_co2_emissions(
                        constants.TYPE_COACH,
                        constants.DEFAULT_CITY,
                        constants.DEFAULT_FUEL,
                        constants.DEFAULT_NB_SEATS,
                        constants.DEFAULT_NB_KM,
                    )
                    * constants.DEFAULT_NB_PASSENGERS
                    * local_distance_m
                )
                step = tmw.Journey_step(
                    i,
                ...
                if not pd.isna(leg.next_departure):
                    step = tmw.Journey_step(
                        ...
                        distance_m=distance(
                            leg.geoloc_destination_seg, leg.next_geoloc
                        ).m,
            ...
            departure_date_formated = dt.strptime(
                str(lst_sections[0].departure_date)[0:15], "%Y-%m-%d %H:%M"
            ).strftime("%Y-%m-%d %H:00")
