...
def format_blablacar_response(rep_json, departure_date, start_point, end_point):
    ...
    trips_waypoints["date_time"] = trips_waypoints.apply(
        lambda x: x["waypoints"]["date_time"], axis=1
    )
    trips_waypoints["date_time"] = pd.to_datetime(trips_waypoints["date_time"])
    trips_waypoints["city"] = trips_waypoints.apply(
        lambda x: x["waypoints"]["place"]["city"], axis=1
    )
    trips_waypoints["address"] = trips_waypoints.apply(
        lambda x: x["waypoints"]["place"]["address"], axis=1
    )
    trips_waypoints["latitude"] = trips_waypoints.apply(
        lambda x: x["waypoints"]["place"]["latitude"], axis=1
    )
    trips_waypoints["longitude"] = trips_waypoints.apply(
        lambda x: x["waypoints"]["place"]["longitude"], axis=1
    )
...
def blablacar_journey(df_response):
    ...
    for trip_id in df_response.trip_id.unique():
        ...
        itinerary["price_leg"] = itinerary.apply(
            lambda x: x.distance_in_meters / x.total_distance * x.price, axis=1
        )
        ...
        for index, leg in itinerary.iterrows():
            ...
            if not pd.isna(leg.next_departure):
                step = TMW.Journey_step(
                    ...
                    distance_m=0,
                    duration_s=(
                        leg["next_departure"] - leg["arrival_date_seg"]
                    ).seconds,
                    price_EUR=[0],
