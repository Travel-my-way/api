...
CARBON_DF_PATH = 'src/emission.csv'


def calculate_co2_emissions(type_transport, nb_km, type_city=None, fuel=None, nb_seats=1):
    logger.info(f"Computing CO2...")
    ...
    if nb_seats is not None:
        filter_nb_seats = (carbon_df[NB_SEATS_MIN] <= float(nb_seats[0])) & (carbon_df[NB_SEATS_MAX] >= float(nb_seats[1]))
